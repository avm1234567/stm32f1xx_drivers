...
const uint8_t inv_sbox[256] = //Inverse Substitution box
{
...
void AES_StatetoString(char *msgchr){
	int k = 0;
	 for(int col = 0;col < 4; col++){
		 for(int row = 0;row < 4; row++){
			 msgchr[k] = state[row][col];
			 k++;
		 }
	 }
	 msgchr[k] = '\0';
    ...
    while(1){
       USART1_SendString("Enter PlainText:");
		...
		//Inverse Cipher
       	...
       	for(int roundinv = 9; roundinv > 0; roundinv--)
       	{
       	    AES_InvShiftRows();
       	    AES_InvSubsByte();
       	    AES_AddRoundKey(roundinv);
       	    AES_InvMixCol();
       	}
       	...
       	USART1_SendString("Inverse State:");
       	USART1_SendString("\r\n");
       	AES_PrintState();
       	USART1_SendString("\r\n");

       	AES_StatetoString(retmsg);
